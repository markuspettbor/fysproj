...
def test(testing = False):
    ...
    #print(planet_pos_t0)
    t0 = 0
    t, pos, vel, mass, fuel_mass, phi, launch_pos = launch(t_load, x_load, v_load, t0, 1/2*np.pi, False)
    t, pos, vel, mass, fuel_mass, phi, launch_pos = launch(t_load, x_load, v_load, t0, 1/2*np.pi-phi, True)
    t1_index = int((t)/dt)
    # print(t*vars.year)
